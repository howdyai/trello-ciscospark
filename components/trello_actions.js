...
TrelloWrapper.prototype.getBoards = function(data) {
  return new Promise((resolve, reject) => {
    this.t.get(`/1/organizations/${this.defaultOrg}/boards`, {
      filter: 'open',
      lists: 'all',
...
TrelloWrapper.prototype.listOrgs = function() {
  return new Promise((resolve, reject) => {
    this.t.get('1/members/me/organizations', {
...
TrelloWrapper.prototype.searchBoard = function(query, opts) {
  ...
  return new Promise((resolve, reject) => {
    if (!boardId) {
      reject({
        message: 'Error, no board default set or provided'
      })
      return
    }
    this.t.get('1/search', {
...
TrelloWrapper.prototype.registerBoardWebhook = function(opts) {
  return new Promise((resolve, reject) => {
    ...
    this.t.post('1/webhooks', {
      idModel: opts.boardId,
      callbackURL: `${WEBHOOK_ROOT}/trello/receive?channel=${opts.channel}&uuid=${webhookUuid}`
    }, (err, data) => {
      if (err) {
        reject(err)
      } else {
...
TrelloWrapper.prototype.updateBoardWebhook = function(opts) {
  return new Promise((resolve, reject) => {
    this.t.put('1/webhooks/' + opts.webhook.id, {
      idModel: opts.boardId,
      callbackURL: `${WEBHOOK_ROOT}/trello/receive?channel=${opts.channel}&uuid=${opts.webhook.uuid}`
    }, (err, data) => {
      if (err) {
        reject(err)
      } else {
